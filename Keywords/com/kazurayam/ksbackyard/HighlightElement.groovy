...
public class HighlightElement {
	...
	public static void pandemic() {
		WebUiBuiltInKeywords.metaClass.'static'.invokeMethod = { String name, args ->
			System.out.println("the primary closure started - name: ${name}, args: ${args}")
			...
			System.out.println("the primary closure finished - name: ${name}, args: ${args}")
			return result
		}
		// one more layer of invokeMethod()
		WebUiBuiltInKeywords.metaClass.'static'.invokeMethod << { String name, args ->
			System.out.println("the overlayed closure started - name: ${name}, args: ${args}")
			def result
			try {
				result = delegate.metaClass.getMetaMethod(name, args).invoke(delegate, args)
			} catch(Exception e) {
			...
			System.out.println("the overlayed closure finished - name: ${name}, args: ${args}")
			return result
		}
	}
